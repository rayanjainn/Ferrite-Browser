...
| T-211 | open | A6/A7 | **Opaque origins have no representation.** `ferrite_core::Origin` accepts only `http`/`https`, because every other scheme has an opaque origin that no `OriginScope` could ever admit. But A6's `RecordingExecutor` has to record the origin of *every* action, including ones taken on `about:blank`, a `data:` URL or a `blob:` URL. Decide the model before A6 needs it: either a distinct `Origin::Opaque` case that no scope admits (so it is always a deviation, like `js.execute`), or a documented rule that such actions are recorded with the last real origin. Found while building `Origin` in A2 (commit `1c01a3e`), which deliberately did not guess. |
