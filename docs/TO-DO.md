...
| T-013 | open | A1 | Docs assume Windows/PowerShell/single dev machine; reality is multi-OS, multi-contributor | `justfile` as the only documented command surface, OS-neutral docs |
...
|---|---|---|---|
| T-101 | open | A1 — Foundation | `[workspace.dependencies]`, build profiles, `justfile`, `deny.toml`, CI matrix rework (Servo weekly not every push). Depends on T-013. |
...
| T-207 | open | A1 (CI gate) / whichever charter next touches each crate | **Workspace is not currently `cargo fmt --check`-clean**: 5 files fail across `ferrite-agent` (`gemini.rs`, `lib.rs`), `ferrite-eval` (`tests/pilot_corpus_validate.rs`), `ferrite-servo` (`session.rs`), `ferrite-shell` (`main.rs`) — despite the old `CLAUDE.md`'s stated "cargo fmt before every commit" rule. Discovered by A0 while testing the new pre-commit hook (`scripts/hooks/pre-commit`), not fixed (those crates are reference material for later charters, not A0 scope; reformatting them piecemeal now would create noisy diffs against code that's being rewritten anyway). A1's CI rework should add a `cargo fmt --check` gate so this can't silently recur; each crate's own rebuild charter (A3 for `ferrite-agent`, A9 for `ferrite-servo`/`ferrite-shell`, A11 for `ferrite-eval`) naturally re-formats as it rewrites. |
