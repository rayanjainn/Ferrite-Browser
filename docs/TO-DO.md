...
| T-213 | open | A3 | **Config loading with env overrides, deferred from A2.** A2's charter line included it, but nothing in `ferrite-core` needs a config *value* — the taxonomy is closed and compile-time, scopes are authored per case, the `Clock` takes none — and building a loading mechanism with no caller is exactly the dead machinery R9 exists to prevent. A3 has the first real values (`FERRITE_MODEL_SMALL`/`FERRITE_MODEL_MAIN`, `FERRITE_MODEL_CALL_BUDGET`, base URL) per §10.2/§10.3, so the mechanism should land there. If it turns out to want a home below the model layer, `ferrite-core` is where it goes — at that point it will have a caller. |
